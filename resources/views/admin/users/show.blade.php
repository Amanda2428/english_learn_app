                ...
                <div class="flex flex-col sm:flex-row items-start sm:items-center">
                    ...
                    <div class="mt-4 sm:mt-0 sm:ml-6 flex-1">
                        ...
                        <div class="flex items-center mt-2">
                            <span class="text-sm text-gray-500">Member since
                                {{ $user->created_at->format('F d, Y') }}</span>
                    ...
                    <div class="mt-4 sm:mt-0 flex space-x-4">
                        <div class="text-center">
                            <p class="text-2xl font-bold text-gray-900">
                                {{ number_format($user->total_points ?? $totalPoints) }}</p>
                ...
                <div class="px-6 py-3 bg-gray-50">
                    <nav class="flex space-x-8">
                        <button onclick="showTab('overview')"
                            class="tab-btn active px-3 py-2 text-sm font-medium text-blue-600 border-b-2 border-blue-600">
                            Overview
                        </button>
                        <button onclick="showTab('progress')"
                            class="tab-btn px-3 py-2 text-sm font-medium text-gray-500 hover:text-gray-700">
                            Learning Progress
                        </button>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="bg-gray-50 rounded-lg p-4 space-y-3">
                                ...
                                <div class="flex justify-between">
                                    ...
                                    <span
                                        class="text-sm font-medium text-gray-900">{{ number_format($user->total_points ?? $totalPoints) }}</span>
                                </div>
                                <div class="flex justify-between">
...
$inProgressSkillsInLevel = $levelProgress
...
if ($completedSkillsInLevel == $totalSkillsInLevel && $totalSkillsInLevel > 0) {
    ...
    $progressColor = 'bg-gray-400';
                                    ...
                                    <div
                                        class="border-2 rounded-xl p-5 {{ $statusClass }} hover:shadow-lg transition-all duration-300">
                                        <!-- Level Header -->
                                        <div class="flex items-center justify-between mb-3">
                                            <div>
                                                ...
                                                <h4 class="text-lg font-bold text-gray-900 mt-1">{{ $level->level_name }}
                                                </h4>
                                            </div>
                                            <span
                                                class="px-3 py-1 rounded-full text-xs font-semibold {{ $statusColor }} bg-white border {{ $statusClass }}">
                                                {{ $statusText }}
                                            </span>
                                        </div>
                                        ...
                                        @if ($level->description)
                                            <p class="text-sm text-gray-600 mb-4 line-clamp-2">{{ $level->description }}
                                            </p>
                                        @endif
                                        ...
                                        <div class="mb-4">
                                            <div class="flex items-center justify-between text-xs mb-1">
                                                ...
                                                <span
                                                    class="font-semibold {{ $statusColor }}">{{ $levelProgressPercentage }}%</span>
                                            </div>
                                        ...
                                        <div class="grid grid-cols-3 gap-2 text-center mb-4">
                                            ...
                                            <div class="bg-white rounded-lg p-2">
                                                <p class="text-lg font-bold text-green-600">{{ $completedSkillsInLevel }}
                                                </p>
                                                <p class="text-xs text-gray-500">Completed</p>
                                            </div>
                                            <div class="bg-white rounded-lg p-2">
                                                <p class="text-lg font-bold text-yellow-600">
                                                    {{ $inProgressSkillsInLevel }}</p>
                                        ...
                                        <div class="flex items-center justify-between border-t border-gray-200 pt-3">
                                            ...
                                            <span
                                                class="font-bold text-purple-600">{{ number_format($pointsInLevel) }}</span>
                                        </div>
                                        ...
                                        @if ($totalSkillsInLevel > 0)
                                            <button onclick="toggleLevelDetails({{ $level->level_id }})"
                                                ...
                                                <svg class="w-4 h-4 ml-1 transition-transform duration-300"
                                                    id="arrow-{{ $level->level_id }}" fill="none"
                                            ...
                                            <div id="level-{{ $level->level_id }}-details" class="hidden mt-4 space-y-3">
                                                @foreach ($level->skills as $skill)
                                                    @php
                                                        $skillProgress = $user->progress
                                                            ->where('skill_id', $skill->skill_id)
                                                            ->first();
                                                        $skillStatus = $skillProgress
                                                            ? $skillProgress->status
                                                            : 'not_started';
                                                        $skillPercentage = $skillProgress
                                                            ? $skillProgress->completion_percentage
                                                            : 0;
                                                        $skillPoints = $skillProgress
                                                            ? $skillProgress->points_earned
                                                            : 0;
                                                    ...
                                                    <div class="bg-white rounded-lg p-3 border border-gray-100">
                                                        <div class="flex items-center justify-between mb-2">
                                                            <span
                                                                class="text-sm font-medium text-gray-800">{{ $skill->skill_name }}</span>
                                                            ...
                                                            <div class="flex-1 mr-3">
                                                                <div class="w-full bg-gray-200 rounded-full h-1.5">
                                                                    <div class="bg-blue-600 h-1.5 rounded-full"
                                                                        style="width: {{ $skillPercentage }}%"></div>
                                                                </div>
                                                            </div>
                                                            <span class="text-purple-600 font-medium">{{ $skillPoints }}
                                                                pts</span>
                                                        </div>
                                                    </div>
                                                @endforeach
                                    ...
                                    @forelse($inProgress as $progress)
                                        <div class="mb-4 last:mb-0 p-3 bg-white rounded-lg border border-yellow-200">
                                            <div class="flex items-center justify-between mb-2">
                                                <div>
                                                    <span
                                                        class="text-sm font-medium text-gray-900">{{ $progress->skill->skill_name ?? 'Unknown Skill' }}</span>
                                                    @if ($progress->level)
                                                        <span
                                                            class="ml-2 text-xs text-gray-500">({{ $progress->level->level_name }})</span>
                                                    @endif
                                                </div>
                                                <span
                                                    class="text-xs px-2 py-1 bg-yellow-100 text-yellow-800 rounded-full">In
                                                    Progress</span>
                                            </div>
                                            ...
                                            <div class="mb-2">
                                                <div class="flex items-center justify-between text-xs mb-1">
                                                    ...
                                                    <span
                                                        class="font-medium text-yellow-600">{{ $progress->completion_percentage }}%</span>
                                                </div>
                                                <div class="w-full bg-gray-200 rounded-full h-1.5">
                                            ...
                                            <div class="flex items-center justify-between text-xs text-gray-500">
                                                <span>Videos:
                                                    {{ $progress->videos_watched }}/{{ $progress->total_videos_in_skill }}</span>
                                                <span>Questions:
                                                    {{ $progress->questions_answered }}/{{ $progress->total_questions_in_skill }}</span>
                                                <span class="text-purple-600 font-medium">{{ $progress->points_earned }}
                                                    pts</span>
                                            </div>
                                        </div>
                                    @empty
                                    ...
                                    @forelse($completed as $progress)
                                        <div class="mb-4 last:mb-0 p-3 bg-white rounded-lg border border-green-200">
                                            <div class="flex items-center justify-between mb-2">
                                                ...
                                                <span
                                                    class="text-xs px-2 py-1 bg-green-100 text-green-800 rounded-full">Completed</span>
                                            </div>
                                            ...
                                            <div class="flex items-center justify-between text-xs text-gray-500">
                                                <span>Videos:
                                                    {{ $progress->videos_watched }}/{{ $progress->total_videos_in_skill }}</span>
                                                <span>Accuracy:
                                                    {{ $progress->questions_answered > 0 ? round(($progress->correct_answers / $progress->questions_answered) * 100) : 0 }}%</span>
                                                <span class="text-purple-600 font-medium">{{ $progress->points_earned }}
                                                    pts</span>
                                            </div>

                                            @if ($progress->completed_at)
                                                <p class="text-xs text-gray-400 mt-2">Completed
                                                    {{ $progress->completed_at->diffForHumans() }}</p>
                                            @endif
                                        </div>
                                    @empty
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-4 gap-4">
                            ...
                            <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-lg p-4 text-white">
                                <p class="text-2xl font-bold">{{ $user->progress->where('status', 'completed')->count() }}
                                </p>
                            ...
                            <div class="bg-gradient-to-br from-yellow-500 to-yellow-600 rounded-lg p-4 text-white">
                                <p class="text-2xl font-bold">
                                    {{ $user->progress->where('status', 'in_progress')->count() }}</p>
                        ...
                        @forelse($recentSessions as $session)
                            <div class="bg-gray-50 rounded-lg p-4">
                                ...
                                <p class="text-sm text-gray-600 mb-2">
                                    ...
                                    Last message
                                    {{ $session->last_msg_at ? $session->last_msg_at->diffForHumans() : 'No messages' }}
                                </p>
