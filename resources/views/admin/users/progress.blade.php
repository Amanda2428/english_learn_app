...
<div class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-blue-600 via-blue-700 to-indigo-800 rounded-2xl shadow-xl overflow-hidden">
        <div class="relative px-6 py-8 sm:px-8 lg:px-10">
            ...
            <div class="absolute inset-0 opacity-10">
                <svg class="absolute left-0 top-0 h-full w-full" viewBox="0 0 100 100" preserveAspectRatio="none">
                    <path d="M0 0 L100 100 M100 0 L0 100" stroke="white" stroke-width="1" vector-effect="non-scaling-stroke"/>
                </svg>
            </div>
            ...
            <div class="relative flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div class="flex items-center space-x-4">
                    <!-- User Avatar -->
                    <div class="h-16 w-16 rounded-full bg-white/20 backdrop-blur flex items-center justify-center ring-4 ring-white/30">
                    ...
                    <div>
                        ...
                        <div class="flex items-center space-x-3 mt-1">
                            ...
                            @if($user->level)
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-400/20 text-yellow-100">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 divide-x divide-blue-500/30 border-t border-blue-500/30">
            ...
            <div class="px-6 py-4">
                ...
                <p class="text-white font-semibold">{{ floor($stats['total_time'] / 60) }}h {{ $stats['total_time'] % 60 }}m</p>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-gradient-to-br from-yellow-400 to-yellow-600 rounded-2xl shadow-lg p-6 text-white">
            ...
            <div class="mt-4 pt-4 border-t border-yellow-500/30">
                <div class="flex justify-between text-sm">
                    ...
                    <span class="font-semibold">{{ $stats['total_time'] > 0 ? round($stats['total_points'] / max(($stats['total_time'] / 60), 1)) : 0 }} pts</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-gradient-to-br from-blue-400 to-blue-600 rounded-2xl shadow-lg p-6 text-white">
            ...
            <p class="text-4xl font-bold mb-2">{{ floor($stats['total_time'] / 60) }}h {{ $stats['total_time'] % 60 }}m</p>
            <p class="text-blue-100 text-sm">Total learning time</p>
            <div class="mt-4 pt-4 border-t border-blue-500/30">
                <div class="flex justify-between text-sm">
                    <span>Daily average</span>
                    <span class="font-semibold">{{ round($stats['total_time'] / max($user->created_at->diffInDays(now()), 1), 1) }} min</span>
                </div>
            </div>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Videos</th>
                        ...
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Started</th>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-medium text-gray-900">{{ $item->skill->skill_name ?? 'N/A' }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            @if($item->level)
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center justify-center">
                                <span class="text-sm font-medium text-gray-700 mr-2">{{ $item->completion_percentage }}%</span>
                                <div class="w-16 bg-gray-200 rounded-full h-2">
                                    <div class="bg-blue-600 h-2 rounded-full" style="width: {{ $item->completion_percentage }}%"></div>
                                </div>
                            </div>
                        </td>
                ...
                <li>
                    <div class="relative pb-8">
                        ...
                        <div class="relative flex space-x-3">
                            <div>
                                <span class="h-8 w-8 rounded-full bg-{{ ['blue', 'green', 'purple', 'yellow'][$index % 4] }}-100 flex items-center justify-center ring-8 ring-white">
                                    @if($activity->skill)
                                    <span class="h-4 w-4 text-{{ ['blue', 'green', 'purple', 'yellow'][$index % 4] }}-600 font-bold text-xs">
                                        {{ substr($activity->skill->skill_name ?? 'S', 0, 1) }}
                                    </span>
                                    @else
                                    <svg class="h-4 w-4 text-{{ ['blue', 'green', 'purple', 'yellow'][$index % 4] }}-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div class="min-w-0 flex-1 pt-1.5 flex justify-between space-x-4">
                                <div>
                                    <p class="text-sm text-gray-600">
                                        <span class="font-medium text-gray-900">{{ $activity->skill->skill_name ?? 'Unknown Skill' }}</span>
                                    ...
                                    <p class="text-xs text-gray-400 mt-1">
                                        Videos: {{ $activity->videos_watched }}/{{ $activity->total_videos_in_skill }} • 
                                        Questions: {{ $activity->questions_answered }}/{{ $activity->total_questions_in_skill }}
                                    </p>
                                </div>
                                <div class="text-right text-sm whitespace-nowrap">
                                    <span class="text-gray-500">{{ $activity->updated_at->diffForHumans() }}</span>
                                    <p class="text-xs text-purple-600 font-medium mt-1">+{{ $activity->points_earned }} pts</p>
                                </div>
                            </div>
                        </div>
                    </div>
