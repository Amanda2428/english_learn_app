...
@section('content')
    @php
        ...
        $totalLessons = $level->skills->sum('level_videos_count');
        $totalQuestionsInLevel = $level->skills->sum('level_questions_count');
        // Define level icons
    ...
    <div class="space-y-8">
        ...
        <div
            class="relative overflow-hidden bg-gradient-to-br from-blue-600 via-indigo-600 to-purple-600 rounded-3xl shadow-2xl">
            <!-- Animated background patterns -->
            <div class="absolute inset-0">
                <div class="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full -mr-20 -mt-20 animate-pulse"></div>
                <div
                    class="absolute bottom-0 left-0 w-48 h-48 bg-white/10 rounded-full -ml-16 -mb-16 animate-pulse delay-1000">
                </div>
                ...
                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                    <div class="flex-1">
                        ...
                        <div class="flex items-center space-x-3 mb-4">
                            <span
                                class="px-4 py-1.5 bg-white/20 backdrop-blur-sm rounded-full text-sm font-semibold inline-flex items-center">
                                <i class="fas fa-flag-checkered mr-2"></i>
                                {{ $level->cefr_level ?? 'Level ' . ($level->level_order ?? 1) }}
                            </span>
                            @if ($levelProgress == 100)
                                <span
                                    class="px-4 py-1.5 bg-green-500/30 backdrop-blur-sm rounded-full text-sm font-semibold inline-flex items-center">
                    ...
                    <div class="hidden lg:block w-64 h-64 bg-white/10 backdrop-blur-sm rounded-2xl p-6">
                        <div
                            class="w-full h-full rounded-xl bg-gradient-to-br from-white/20 to-white/5 flex items-center justify-center">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                ...
                <div class="flex space-x-2 bg-gray-100 p-1 rounded-lg">
                    <button
                        class="filter-btn px-4 py-2 text-sm font-medium rounded-md bg-white text-gray-900 shadow-sm transition-all"
                        data-filter="all">
                        All Skills
                    </button>
                    <button
                        class="filter-btn px-4 py-2 text-sm font-medium rounded-md text-gray-600 hover:text-gray-900 transition-colors"
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6" id="skills-grid">
                @forelse($level->skills as $skill)
                    @php
                        ...
                        $videosCount = $skill->level_videos_count;
                        $questionsCount = $skill->level_questions_count;
                    ...
                    <a href="{{ route('user.skills.show', $skill) }}"
                        ...
                        data-status="{{ $status }}">
                        <div
                            class="bg-white border-2 border-gray-100 rounded-xl overflow-hidden hover:border-{{ $config['color'] }}-200 hover:shadow-xl transition-all hover:-translate-y-1">
                            <div class="p-6">
                                <!-- Status bar -->
                                <div class="flex items-center justify-between mb-4">
                                    <div class="flex items-center space-x-2">
                                        <span
                                            class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-{{ $config['bg'] }} text-{{ $config['text'] }}">
                                            <i class="fas {{ $config['icon'] }} mr-1.5 text-xs"></i>
                                            {{ $config['label'] }}
                                        </span>
                                        @if ($status === 'completed')
                                            <span
                                                class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-yellow-50 text-yellow-600">
                                ...
                                <div class="flex items-start space-x-4 mb-4">
                                    <div
                                        class="w-14 h-14 rounded-xl bg-{{ $config['bg'] }} flex items-center justify-center group-hover:scale-110 transition-transform">
                                        <i class="fas {{ $skillIcon }} text-{{ $config['text'] }} text-2xl"></i>
                                    </div>
                                    <div class="flex-1">
                                        <h3
                                            class="text-lg font-bold text-gray-900 group-hover:text-{{ $config['color'] }}-600 transition-colors mb-1">
                                            {{ $skill->skill_name }}
                                        </h3>
                                ...
                                <div class="grid grid-cols-3 gap-3 mb-4">
                                    ...
                                    <div class="text-center p-2 bg-gray-50 rounded-lg">
                                        <div class="text-sm font-bold text-gray-900">{{ $questionsCount * 10 }}</div>
                                        <div class="text-xs text-gray-500 flex items-center justify-center">
                                ...
                                @if ($status === 'in_progress' && $completionPercentage > 0)
                                    <div class="mb-4">
                                        <div class="flex justify-between text-xs mb-1.5">
                                            ...
                                            <span
                                                class="font-bold text-{{ $config['color'] }}-600">{{ $completionPercentage }}%</span>
                                        </div>
                                ...
                                <div class="flex items-center justify-between pt-4 border-t border-gray-100">
                                    <span
                                        class="text-sm font-semibold text-{{ $config['color'] }}-600 group-hover:underline flex items-center">
                                        {{ $config['action'] }}
                                        <i
                            ...
                            <!-- Color bar at bottom -->
                            <div
                                class="h-1.5 bg-gradient-to-r from-{{ $config['color'] }}-500 to-{{ $config['color'] }}-400 w-0 group-hover:w-full transition-all duration-500">
                            </div>
                        </div>
                    </a>
                @empty
                    <div class="col-span-2">
                        <div
                            ...
                            <p class="text-gray-500 mb-6 max-w-md mx-auto">We're crafting engaging lessons for this level.
                                Check back soon to start learning!</p>
                ...
                <div class="relative flex justify-between">
                    @foreach ($level->skills->take(5) as $index => $skill)
                        ...
                        <div class="relative flex flex-col items-center">
                            ...
                            <span
                                class="text-sm font-medium text-center max-w-[100px] {{ $nodeStatus != 'Not Started' ? 'text-white' : 'text-gray-500' }}">
                                {{ $skill->skill_name }}
                            </span>
                        </div>
                    @endforeach
        ...
        <div class="flex justify-between items-center mt-8">
            ...
            @if ($levelProgress == 100)
                <div class="flex items-center space-x-3">
                    ...
                    <a href="{{ route('user.levels.next', $level) }}"
                        class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-xl hover:from-purple-700 hover:to-pink-700 transition-colors group">
                        Next Level
